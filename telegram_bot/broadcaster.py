...
def _build_caption(product: dict) -> str:
    ...
    desc_text = desc_am or short_desc_am
    ...
    if compare_price and float(compare_price) > price:
        discount_pct = round((1 - price / float(compare_price)) * 100)
        lines.append(f'💰 ዋጋ: {price:,.0f} ብር  ~~{float(compare_price):,.0f} ብር~~ ? {discount_pct}% ቅናሽ! 🎉')
    else:
        lines.append(f'💰 ዋጋ: {price:,.0f} ብር')
    ...
    lines += [
        '',
        '━' * 30,
        f'📍 አድራስ: {STORE_LOCATION}',
        f'📞 ስልክ: {STORE_PHONE}',
        '',
        '💬 ተገማሪ መረጃ ይፈልጋሉ? ልይዉን ይጋድ!',
    ]
...
async def _send_photo_to_user(client: httpx.AsyncClient, chat_id, photo_url: str, caption: str, keyboard: dict) -> bool:
    """Send a product photo with caption to a single Telegram user."""
    api_url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}"

    if photo_url.startswith('/media/'):
        full_photo_url = photo_url.split('/media/', 1)[-1]
